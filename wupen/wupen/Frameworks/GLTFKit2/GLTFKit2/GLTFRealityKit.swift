...
public class GLTFRealityKitLoader {
    ...
    public static func convert(scene: GLTFScene) -> RealityKit.Entity {
        ...
        if #available(iOS 15.0, *) {
            ...
            return instance.convert_13(scene: scene)
        }
        
    }

    @available(macOS 12.0, iOS 13.0, *)
    func convert_13(scene: GLTFScene) -> RealityKit.Entity {
        let context = GLTFRealityKitResourceContext()

        let rootEntity = Entity()

        do {
            let rootNodes = try scene.nodes.compactMap { try self.convert_13(node: $0, context: context) }
    ...
    @available(macOS 12.0, iOS 13.0, *)
    func convert_13(node gltfNode: GLTFNode, context: GLTFRealityKitResourceContext) throws -> RealityKit.Entity {
...
//        }
        ...
        for childNode in gltfNode.childNodes {
            nodeEntity.addChild(try convert_13(node: childNode, context: context))
        }
