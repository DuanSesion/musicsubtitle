...
public typealias MKProgressHUD = ProgressHUD
...
public extension ProgressHUD {
    ...
    //-------------------------------------------------------------------------------------------------------------------------------------------
    class func showLoading(_ icon: AlertIcon, interaction: Bool = false) {

        DispatchQueue.main.async {
            shared.setup(status: nil, staticImage: nil, hide: false, interaction: interaction)
        }
    }
    
    //-------------------------------------------------------------------------------------------------------------------------------------------
    class func showHUD(_ status: String? = nil, icon: AlertIcon, interaction: Bool = false, delay: TimeInterval? = nil) {
        DispatchQueue.main.async {
            shared.setup(status: status, staticImage: nil, hide: false, interaction: interaction)
            shared.toolbarHUD?.removeFromSuperview()
            shared.viewProgress?.removeFromSuperview()
            shared.viewAnimation?.removeFromSuperview()
            
            if shared.staticImageView == nil {
                shared.staticImageView = UIImageView(frame: CGRect(x: 0, y: 0, width: 165, height: 165))
                shared.staticImageView?.contentMode = .scaleAspectFit
                shared.staticImageView?.animationImages = shared.animationImages
                shared.staticImageView?.startAnimating()
            }

            shared.staticImageView?.frame = CGRect(x: 0, y: 0, width: 165, height: 165)
            shared.staticImageView?.backgroundColor = .clear
            shared.viewBackground?.addSubview(shared.staticImageView!)
            ...
            shared.staticImageView?.center = CGPoint(x: centerX, y: centerY)
            shared.viewBackground?.backgroundColor = UIColor(red: 0, green: 0, blue: 0, alpha: 0.5)
        }
    }
}
...
public class ProgressHUD: UIView {
    ...
    lazy var animationImages: [UIImage] = {
        ...
        for i in 0..<47 {
            let img:UIImage? = UIImage(named: String(format: "Loading_%05ld", i))
            if let im = img {
                animationImages.append(im)
            }
        }
    ...
    public class func hudHide() {
        DispatchQueue.main.async {
            if (shared.alpha == 1) {
                UIView.animate(withDuration: 0.15, delay: 0, options: [.allowUserInteraction, .curveEaseIn], animations: {
                    shared.toolbarHUD?.transform = CGAffineTransform(scaleX: 0.3, y: 0.3)
                    shared.toolbarHUD?.alpha = 0
                    shared.viewBackground?.alpha = 0
