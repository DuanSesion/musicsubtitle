...
@property (nonatomic, assign) CGFloat beginDragginX;
...
- (void)scrollViewWillBeginDragging:(UIScrollView *)scrollView
{
    ...
    self.isTouch = true;
    self.beginDragginX = scrollView.contentOffset.x;
}
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView
{
    ...
    int itemIndex = [self currentIndex];
    int indexOnPageControl = [self pageControlIndexWithCurrentCellIndex:itemIndex];
    CGFloat endDragginX = scrollView.contentOffset.x;
    CGFloat offsetX = endDragginX - self.beginDragginX;
    if (offsetX > 0) {//左滑动
        if ([self.delegate respondsToSelector:@selector(cycleScrollView:left:)]) {
            [self.delegate cycleScrollView:self left:indexOnPageControl];
        }
        
    } else if (offsetX < 0) {//右滑动
        if ([self.delegate respondsToSelector:@selector(cycleScrollView:right:)]) {
            [self.delegate cycleScrollView:self right:indexOnPageControl];
        }
    }
    self.isTouch = false;
}
