...
class WPAttributeMarkView: UIImageView {
    
    public var closeBlock:(()->Void)?
    public var nextBlock:(([String])->Void)?
    ...
    lazy var nextButton: UIButton = {
        ...
        let bgimg = UIImage.wp.createColorImage(rgba(240, 140, 37), size: CGSize(width: 60, height: 60)).wp.roundCorner(30)
        nextButton.setBackgroundImage(bgimg, for: .normal)
        
        let colors = [rgba(188, 188, 188, 1).cgColor,rgba(230, 230, 230, 1).cgColor]
        let bgimgEN = UIImage.wp.jianBianLeftToRight(colors: colors, size: CGSize(width: 60, height: 60))?.wp.roundCorner(30)
...
extension WPAttributeMarkView:UICollectionViewDelegateFlowLayout {
    func collectionView(_ collectionView: UICollectionView, layout collectionViewLayout: UICollectionViewLayout, sizeForItemAt indexPath: IndexPath) -> CGSize {
        return WPRemarkCell.cellSize(sizeForItemAt: indexPath, datas: datas)
    }
}

extension WPAttributeMarkView:UICollectionViewDelegate {
...
extension WPAttributeMarkView:UICollectionViewDataSource {
