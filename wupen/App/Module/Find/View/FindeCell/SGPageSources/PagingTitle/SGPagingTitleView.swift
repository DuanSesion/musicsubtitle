...
public class SGPagingTitleView: UIView {
    ...
    public var resetTitles:[String]? {//MARK: 重制标题
        didSet {
            tempBtns.forEach { btn in
                btn.removeFromSuperview()
            }
            tempBtns.removeAll()
            
            self.titles = self.resetTitles ?? []
            addTitleBtns()
            self.reset(index: 0)
        }
    }
    ...
    private lazy var indicator: UIImageView = {
        let tempIndicator = UIImageView()
        tempIndicator.backgroundColor = configure.indicatorColor
        tempIndicator.contentMode = .scaleAspectFit
...
private extension SGPagingTitleView {
    ...
    func addTitleBtns() {
        ...
        indicator.image = configure.indicatorImg
        ...
        for idx in 0..<titleCount {
            ...
            btn.layer.cornerRadius = configure.radues
            btn.backgroundColor = configure.itemNomarlColor
        ...
        if allBtnWidth <= frame.size.width {
            ...
            let btnW: CGFloat = max(55,frame.size.width / CGFloat(titleCount))
    ...
    func fromLeftToRightLayout() {
        ...
        scrollView.contentSize = CGSize(width: (lastBtn?.frame.maxX ?? 0) , height: frame.size.height)
    }
}
// MARK: 标题按钮的点击事件
private extension SGPagingTitleView {
    ...
    func changeSelectedBtn(btn: UIButton) {
        btn.backgroundColor = configure.itemSelectedColor
        ...
        if selectedFontName == defaultFontName && selectedFontPointSize == defaultFontPointSize {
            ...
            if configure.gradientEffect {
                tempBtns.forEach { (EBtn) in
                    ...
                    EBtn.backgroundColor = configure.itemNomarlColor
                }
                btn.titleLabel?.textColor = configure.selectedColor
