...
public class SGPagingContentScrollView: SGPagingContentView {
    ...
    public var resetChirdControllers:[UIViewController]? {//MARK: 重置页面
        didSet {
            ...
            self.childViewControllers.append(contentsOf: resetChirdControllers ?? [])
            
            let contentWidth: CGFloat = CGFloat(childViewControllers.count) * scrollView.frame.size.width
            scrollView.contentSize = CGSize(width: contentWidth, height: 0)
            self.previousChildVCIndex = -1
            setPagingContentScrollView(index: 0)
        }
    }
    ...
    private lazy var scrollView: SGPagingScrollView = {
        let tempScrollView = SGPagingScrollView()
