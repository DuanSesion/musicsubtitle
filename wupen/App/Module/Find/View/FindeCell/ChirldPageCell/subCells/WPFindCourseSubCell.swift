...
class WPFindCourseSubCell: UICollectionViewCell {
    class func collectionViewWithCell(_ collectionView:UICollectionView,indexPath:IndexPath,datas:[WPCouserModel])->UICollectionViewCell {
        let cell:WPFindCourseSubCell = collectionView.dequeueReusableCell(withReuseIdentifier: "cell", for: indexPath) as! WPFindCourseSubCell
    ...
    class func didSelectItemAt(_ collectionView:UICollectionView,indexPath:IndexPath,datas:[WPCouserModel],tags:String?=nil, controller:WPFindCourseController)->Void {
        if datas.count > indexPath.row {
            let model = datas[indexPath.row]
            let vc:WPCourseDetailController = WPCourseDetailController()
            vc.model = model
            controller.navigationController?.pushViewController(vc, animated: true)
            WPUmeng.um_event_find_CourseDetailsFromDiscover()
            
            if controller.model == nil {
                WPUmeng.um_event_find_HotCourseClick(model.lectureId)
            }
        }
    }
