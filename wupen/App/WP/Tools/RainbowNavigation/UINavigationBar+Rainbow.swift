...
//            navigationView!.backgroundColor = newValue
            if let c = newValue {
                let img = UIImage.wp.createColorImage(c, size: CGSize(width: navigationBar.bounds.width, height: navigationBar.bounds.height + UIApplication.shared.statusBarFrame.height))
//                navigationBar.setBackgroundImage(img, for: .any, barMetrics: .default)
                navigationBar.layer.contents = img.cgImage
            }
    ...
    public func clear() {
        ...
        navigationBar.layer.contents = nil
