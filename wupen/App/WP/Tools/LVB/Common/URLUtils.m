...
//    NSString *hexTxTime = [[NSString alloc] initWithFormat:@"%d", (int)time];
...
//    NSString *url = [NSString stringWithFormat:@"rtmp://%@/wupenapp/%@",
//           PUSH_DOMAIN, streamId];
    return url;
}
...
+ (NSString*)generateTRTCPushUrl:(NSString*)streamId userId:(NSString*)userId {
    NSString *url = [NSString stringWithFormat:@"trtc://cloud.tencent.com/push/%@?sdkappid=%d&userid=%@&usersig=%@&appscene=live",
                streamId, SDKAppID, userId, [GenerateUserSig genTestUserSig:userId]];
    return url;
}
