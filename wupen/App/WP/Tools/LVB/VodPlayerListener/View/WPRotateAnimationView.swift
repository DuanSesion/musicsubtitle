...
class WPRotateAnimationViewConfigure: NSObject {
    /// 开始起点
    ...
    /// 小球背景颜色
    var backgroundColor: UIColor =  UIColor.init(red: 254/255.0, green: 143/255.0, blue: 11/255.0, alpha: 1)  

    fileprivate class func defaultConfigure() -> WPRotateAnimationViewConfigure {
        let configure = WPRotateAnimationViewConfigure()
        return configure
    }
}

class WPRotateAnimationView: UIView {
    /// 默认配置
    private let configure = WPRotateAnimationViewConfigure.defaultConfigure()
    /// layer数组
...
extension WPRotateAnimationView {
    ...
    /// 更新配置
    func updateWithConfigure(_ configureBlock: ((WPRotateAnimationViewConfigure) -> Void)?) {
